...
interface WarningIconContainerProps extends BoxProps {
...
})<WarningIconContainerProps>(({ theme }) => ({
  display: "flex",
  alignItems: "center",
...
const getIssueBorderColor = (category: string) => {
  const theme = useTheme();
  const isDark = theme.palette.mode === "dark";
...
type ResultsMap = Record<string, any[]> | null;
...
export default function AnalysisResults({ results }: AnalysisResultsProps) {
  ...
  return (
    <ResultsContainer elevation={0}>
      ...
      {Object.entries(results as Record<Category, any[]>).map(
        ([category, issues]) => {
          ...
          return (
            <StyledAccordion
              key={category}
              defaultExpanded={category === "dbLoops"}
            >
              <AccordionSummary
                expandIcon={
                  <ExpandMoreIcon
              ...
              >
                <CategoryHeader>
                  ...
                  <IssueCountChip label={issues.length} size="small" />
                </CategoryHeader>
              </AccordionSummary>
                ...
                {issues.map((issue, index) => (
                  <IssueItem
                    ...
                    sx={{
                      ...
                      ...(isDark && {
                        borderLeft: `3px solid ${getIssueBorderColor(
                          category
                        )}`,
                  ...
                  >
                    <WarningIconContainer
                      categoryColor={getIssueBorderColor(category)}
                    >
                      <WarningIcon
                        fontSize="small"
                        sx={{
                    ...
                    <IssueContent>
                      <IssueFilepath>{issue.filepath}</IssueFilepath>
                      ...
                      <IssueDetails>
                        ...
                        <DetailChip>{issue.name}</DetailChip>
                      </IssueDetails>
                    </IssueContent>
                  </IssueItem>
                ))}
